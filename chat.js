...
//   3. OpenRouter   — OPENROUTER_API_KEY  (openrouter.ai, free, no card)
...
//
// A note on free-tier model names: they rotate and get deprecated more
// often than you'd expect (this is what broke chat here previously — two
// of the four models below had been retired by their providers). Each one
// can be overridden without a code change via GROQ_MODEL / GEMINI_MODEL /
// OPENROUTER_MODEL / CEREBRAS_MODEL env vars if a default below 404s again
// before this file gets updated.
...
export default async function handler(req, res) {
  ...
  // Free-tier model catalogs on these providers rotate and get deprecated
  // fairly often (this is exactly what broke chat here — two of the four
  // model names below had been retired by their providers since this file
  // was first written). Each one can be overridden with an env var without
  // touching code, so a future rotation is a one-line fix in Vercel's
  // dashboard instead of a redeploy.
  const providers = [
    {
      name: 'groq',
    ...
    },
    {
      ...
      call: signal => callGemini({
        ...
        // points at their current free-tier Flash model, so it shouldn't
        // go stale the way a pinned version number does.
    ...
    },
    {
      ...
      call: signal => callOpenAICompatible({
        ...
        // "openrouter/free" is OpenRouter's own router model — it picks
        // from whatever free models are actually available right now,
        // instead of us hardcoding one specific ":free" model that can
        // rotate out of the free catalog with no warning.
